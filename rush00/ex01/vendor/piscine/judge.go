...
func Judge(board []string) string {
	...
	for i := -1; i < 2; i++ {
		for j := -1; j < 2; j++ {
			curI, curJ := chess.king[0]+i, chess.king[1]+j
			curPiece := chess.board[[2]int{curI, curJ}]
			if (i == 0 && j == 0) || !isInBoard(curI, curJ, chess.size) {
				continue
			}
			chess.board[[2]int{curI, curJ}] = 'K'
			chess.board[[2]int{chess.king[0], chess.king[1]}] = '.'
			chess.king[0], chess.king[1] = curI, curJ
			if !chess.isInCheck() {
				if isInCheck {
					return "InCheck"
				} else {
			...
			chess.board[[2]int{curI, curJ}] = curPiece
			chess.king[0], chess.king[1] = curI-i, curJ-j
			chess.board[[2]int{chess.king[0], chess.king[1]}] = 'K'
		}
	}
	if isInCheck {
		return "Checkmate"
...
func convertBoard(board []string) map[[2]int]rune {
	...
	for i, _ := range board {
		for j, c := range []rune(board[i]) {
			if c == 'P' && i == 0 {
				newBoard[[2]int{i, j}] = 'Q'
			} else if c == '.' || c == 'K' || c == 'P' || c == 'B' || c == 'R' || c == 'Q' {
